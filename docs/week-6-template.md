...
Опциональные "заявленные" фичи: уведомление об устаревшем пароле, проверка по базам утечек, Digital Inheritance (передача доступа доверенному лицу при неактивности владельца — пока только спека, без кода).
