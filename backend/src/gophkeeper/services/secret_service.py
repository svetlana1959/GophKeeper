...
class SecretService:
    ...
    async def store(self, *, account_id: str, secret_id: UUID, ciphertext: bytes) -> Secret:
    ...
    async def update(self, *, secret_id: UUID, ciphertext: bytes, base_version: int) -> Secret:
        async with self._uow as uow:
    ...
    async def fetch(self, secret_id: UUID) -> Secret:
        async with self._uow as uow:
